...
void ServerDecorationPlugin::onMouseButton(uint32_t button, bool pressed, int mx, int my) {
    ...
    if (m_hoveredView && m_hoveredButton) {
        if (m_hoveredButton == (void*)1) {
            handleCloseClick(m_hoveredView);
        } else if (m_hoveredButton == (void*)2) {
            handleMaximizeClick(m_hoveredView);
        } else if (m_hoveredButton == (void*)3) {
...
void ServerDecorationPlugin::renderBorder(OverlayRenderer* renderer, const WindowDecoration& deco) {
    ...
    const float* color = deco.focused ? WindowDecoration::FOCUSED_BG : WindowDecoration::UNFOCUSED_BG;
...
void ServerDecorationPlugin::renderButtons(OverlayRenderer* renderer, const WindowDecoration& deco) {
    ...
    bool closeHover = (m_hoveredView == deco.view && m_hoveredButton == (void*)1);
    renderer->drawRect(
    ...
    bool maxHover = (m_hoveredView == deco.view && m_hoveredButton == (void*)2);
    renderer->drawRect(
        (float)btnX, (float)btnY, (float)btnSize, (float)btnSize,
    ...
    // Minimize button
    btnX -= btnSize + spacing;
    bool minHover = (m_hoveredView == deco.view && m_hoveredButton == (void*)3);
    renderer->drawRect(
        (float)btnX, (float)btnY, (float)btnSize, (float)btnSize,
...
void* ServerDecorationPlugin::buttonAtPosition(const WindowDecoration& deco, int mx, int my) {
    int btnSize = 20;
    int spacing = 8;
    int btnY = deco.y - WindowDecoration::TITLE_BAR_HEIGHT + 6;
    int btnX = deco.x + deco.width - btnSize - spacing;
    ...
    btnX -= btnSize + spacing;
    if (mx >= btnX && mx <= btnX + btnSize && my >= btnY && my <= btnY + btnSize) {
        return (void*)2;  // Maximize
    }
    
    btnX -= btnSize + spacing;
    if (mx >= btnX && mx <= btnX + btnSize && my >= btnY && my <= btnY + btnSize) {
        return (void*)3;  // Minimize
    }
