...
#include "SceneGraphOptimized.hpp"
#include <wm/bridge.h>
#include <Logger.h>
#include <stdlib.h>
#include <string.h>
...
bool scene_node_add_child(SceneNode* parent, SceneNode* child, char* error_out, size_t error_size) {
    ...
    if (scene_detect_loop_opt(parent, child)) {
...
bool scene_node_remove_child_opt(SceneNode* parent, SceneNode* child, char* error_out, size_t error_size) {
    if (!parent || !child) {
        snprintf(error_out, error_size, "NULL parent or child");
        return false;
    }
...
bool scene_node_reparent_opt(SceneNode* node, SceneNode* new_parent, char* error_out, size_t error_size) {
    if (!node || !new_parent) {
    ...
    // Check for loops
    if (scene_detect_loop_opt(new_parent, node)) {
        snprintf(error_out, error_size, "Loop detected");
        return false;
    }
    ...
    if (node->parent) {
        if (!scene_node_remove_child_opt(node->parent, node, error_out, error_size)) {
            return false;
        }
    }
...
bool scene_detect_loop_opt(SceneNode* start, SceneNode* potential_ancestor) {
...
bool scene_validate_opt(Scene* scene, char* error_out, size_t error_size) {
    if (!scene) {
...
void scene_print_tree_opt(Scene* scene) {
    if (!scene) {
...
char* scene_node_get_path_opt(SceneNode* node, char* buffer, size_t buffer_size) {
